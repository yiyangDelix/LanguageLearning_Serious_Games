...
namespace UnityEngine.UI.TableUI
{
    ...
    public class TextProperties : MonoBehaviour
    {
        ...
        void ApplyProperty(string oriProp, string targetProp, Action<TextMeshPro> extraAction = null)
        {
            ...
            for (int i = min.x; i < max.x; i++)
            {
                for (int j = min.y; j < max.y; j++)
                {
                    try
                    {
                        TextMeshPro tmp = tu.data[j].list[i];
                        typeof(TextMeshPro).GetProperty(targetProp).SetValue(tmp, prop.GetValue(this));
        ...
        internal void CopyAllValues(TextMeshPro tmp)
        {
            tmp.font = FontAsset;
